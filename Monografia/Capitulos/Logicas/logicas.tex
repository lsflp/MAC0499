...
As LD possuem um jeito próprio de organizar o conhecimento. Elas utilizam de três definições para retratar o que é desejado\cite{logicaMatos}. Cada uma delas é utilizada para a construção de uma ontologia. São, a seguir:
...
Esta linguagem contempla os seguintes construtores, para quaisquer conceitos \texttt{A} e \texttt{B} e qualquer papel \texttt{r}:
...
Essa linguagem também permite que sejam escritos axiomas terminológicos, tais como a subsunção, denotada por $ \texttt{A} \sqsubseteq \texttt{B} $, e a equivalência, caracterizada como $ \texttt{A} \equiv \texttt{B} $ e que significa $ \texttt{A} \sqsubseteq \texttt{B} $ e $ \texttt{B} \sqsubseteq \texttt{A} $. Para o conhecimento assertivo também existem axiomas, tanto para conceitos ($\texttt{A}(x)$), quanto para papéis ($\texttt{r}(x,y)$).

A linguagem $\mathcal{ALC}$ permite que seja feita a negação de conceitos complexos (não-atômicos). Tal propriedade permite descobrir, por exemplo, se algum conceito é {\color{red}satisfazível}. Usando o símbolo $ \sim $ para demonstrar equivalência, teremos que:
...
A partir de sua constituição, é possível definir três {\color{red}sub-linguagens} da $\mathcal{ALC}$\cite{logicaSchmidt}. Tais {\color{red}sub-linguagens} são descritas a seguir.
...
\begin{itemize}
	...
	\item $\mathcal{AL}$: é a intersecção das {\color{red}sub-linguagens} acima.
\end{itemize}

Os nomes das {\color{red}sub-linguagens} acima são derivados da seguinte maneira: $ \mathcal{ALU} $ vem da junção de $ \mathcal{AL} $ com uniões, $ \mathcal{ALE} $ é a $ \mathcal{AL} $ acrescida de quantificadores existenciais. A própria $ \mathcal{ALC} $ é uma extensão da $ \mathcal{AL} $ com adição de complementos (negações) para qualquer conceito, seja ele atômico ou complexo. 

A $ \mathcal{ALC} $ é base para outras linguagens mais expressivas, que são construídas a partir de sua extensão. Para denotá-las, são usadas letras em suas siglas, que mostram as propriedades que as diferenciam entre si. Algumas delas são:
...
Uma interpretação $ \mathcal{I} = (D^{\mathcal{I}}, \cdot^{\mathcal{I}})$ consiste em um conjunto domínio ($ D^{\mathcal{I}} $) e uma função de interpretação $ \cdot^{\mathcal{I}} $ que atribui a toda descrição de conceito um subconjunto de $ D^{\mathcal{I}} $, seguindo as seguintes equações\cite{logicaHorrocks}:

\begin{itemize}
	\item $ \top^\mathcal{I} = D^\mathcal{I} $
	\item $ \bot^\mathcal{I} = \emptyset $
	\item $ (\neg \texttt{A})^\mathcal{I} = D^\mathcal{I} \setminus \texttt{A}^\mathcal{I}  $
	...
	\item $(\exists \texttt{r.A})^\mathcal{I} = \{a \in D^\mathcal{I} | \exists(a, b) \in \texttt{r}^\mathcal{I} \text{ e } b \in \texttt{A}^\mathcal{I}\}$  
	\item $(\forall \texttt{r.A})^\mathcal{I} = \{a \in D^\mathcal{I} | \forall(a, b) \in \texttt{r}^\mathcal{I} \text{ e } b \in \texttt{A}^\mathcal{I}\}$
\end{itemize}
...
Com isso, podemos definir Consequência Lógica como $ \Sigma \models \phi $ se todo modelo de $ \Sigma $ é um modelo de $ \phi $. Em nossa ontologia, se tivermos a \textit{TBox}: $ \exists \texttt{HinoNacional} \sqsubseteq \texttt{Musica}$ e a \textit{ABox}: \texttt{HinoNacional(hinoNacionalBrasileiro)}, $ \Sigma \models \texttt{Musica(hinoNacionalBrasileiro)} $ será uma consequência possível.

\section{Consistência}

Após uma base de conhecimento ser feita, é necessário que ela seja consistente, ou seja, se ela não contradiz a si própria, no caso que podemos inferir um conceito e sua negação ao mesmo tempo. 
...
\begin{enumerate}
	\item a regra E (ou \textit{$ \sqcap $-rule}): se $(\texttt{A} \sqcap \texttt{B})(a)$ está em $ \mathcal{A} $, mas \texttt{A}$ (a) $ e \texttt{B}$ (a) $ não estão os dois em $ \mathcal{A} $, então a \textit{ABox} $ \mathcal{A} $ será acrescida de \texttt{A}$ (a) $ e \texttt{B}$ (a) $.
	\item a regra OU (ou \textit{$ \sqcup $-rule}): quando $(\texttt{A} \sqcup \texttt{B})(a)$ está em $ \mathcal{A} $, mas nem \texttt{A}$ (a) $ nem \texttt{B}$ (a) $ está em $ \mathcal{A} $, então a \textit{ABox} $ \mathcal{A} $ será acrescida de \texttt{A}$ (a) $ ou de \texttt{B}$ (a) $.
	\item a regra EXISTE (ou \textit{$ \exists $-rule}): se $(\exists \texttt{r.A})(a)$ está em $ \mathcal{A} $, e não há nenhum indivíduo $ c $ tal que \texttt{r}$ (a,c) $ e \texttt{A}$ (c) $ estejam em $ \mathcal{A} $, então tanto \texttt{r}$ (a,b) $ quanto \texttt{A}$ (b) $ serão adicionados a $ \mathcal{A} $, onde $ b $ é um novo indivíduo.
	\item a regra PARA TODO (ou \textit{$ \forall $-rule}): quando se $(\forall \texttt{r.A})(a)$ e \texttt{r}$ (a,b) $ estão em $ \mathcal{A} $, mas \texttt{A}$ (c) $ não está, esse último será colocado em $ \mathcal{A} $.  
\end{enumerate}

Para descobrir se uma fórmula é insatisfazível, basta aplicar essas regras na sentença lógica. Se no final chegarmos em um \textit{Clash}, ou seja, um choque, que é quando temos um conceito e sua negação na \textit{ABox}, a sentença é insatisfazível.
...
Como um \textit{Clash} foi atingido, a sentença é insatisfazível.
...
Para verificar que sentenças no estilo $ \texttt{C} \sqsubseteq \texttt{D} $ são válidas, podemos verificar a satisfazibilidade de $ \texttt{C} \sqcap \neg \texttt{D} $. Se essa última for insatisfazível, a primeira é válida.