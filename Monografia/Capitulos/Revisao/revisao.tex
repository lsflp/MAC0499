...
Existe um construtor para a operação da Revisão, que será discutido mais à frente.
...
\begin{center}
	$ K \ast \alpha = $ {\color{red} Perguntar pro Fillipe},
\end{center}

Essa operação satisfaz os seguintes postulados:

\begin{description}
	\item[Inclusão] $ K \ast \alpha \subseteq K + \alpha $ \\
	...
	\item[Sucesso] $ \alpha \in K \ast \alpha $\\
	Também chamado de sucesso forte, obriga que $ \alpha $ esteja no conjunto gerado pela operação.
	\item[Consistência fraca] Se $ \alpha $ é consistente/coerente, então $ K \ast \alpha $ será consistente/coerente. Se uma base de conhecimento é inconsistente, então existe algum conceito nela que é necessariamente vazio. Essa noção é chamada de coerência.
	\item[\textit{Core-retainment}] Se $ \beta \in K \setminus K \ast \alpha $, então existe $ K' \subseteq K \cup \{\alpha\} $ tal que $ K' $ é consistente/coerente e $ K' \cup \{\beta\}$ não é consistente/coerente.
	\item[Pré-expansão] $ K + \alpha \ast \alpha = K \ast \alpha $ \\
